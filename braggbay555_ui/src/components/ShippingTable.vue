        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Shippings</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalShippings = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalShippings">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Shipping</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="ShippingId" type="text" placeholder="Enter ShippingId" v-model="shippingToAdd.shippingId"></base-input>
  <base-input label="TrackingNumber" type="text" placeholder="Enter TrackingNumber" v-model="shippingToAdd.trackingNumber"></base-input>
  <base-input label="ShippingDate" type="text" placeholder="Enter ShippingDate" v-model="shippingToAdd.shippingDate"></base-input>
  <base-input label="DeliveryDate" type="text" placeholder="Enter DeliveryDate" v-model="shippingToAdd.deliveryDate"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="shippings" :row-key="record => record.ShippingId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <ShippingPictureView :shippings="shippings" />
          </div>
...
import ShippingPictureView from './ShippingPictureView.vue';
...
export default {
    ...
    return {
      ...
      columns: [
		...
		dataIndex: 'shippingId',
          visible: true,
          scopedSlots: { customRender: 'shippingId' },
          sorter: true
          //sorter: (a, b) => a.shippingId - b.shippingId,
		...
		dataIndex: 'shippingDate',
          visible: true,
          scopedSlots: { customRender: 'shippingDate' },
          sorter: true
          //sorter: (a, b) => a.shippingDate - b.shippingDate,
          //sorter: (a, b) => a.shippingDate.localeCompare(b.shippingDate),
        },
        {
          title: 'Delivery Date',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} shippings`,
      },
      ...
      sortBy: 'shippingId',           // Column to sort by
  ...
  methods: {
    ...
    async renderShippingsTable() {
      ...
      let shippingsTableData = [];
      for (let i = 0; i < this.shippings.length; i++) {
        shippingsTableData.push({
          id: i,
          shippingId: this.shippings[i].shippingId,
          year: this.shippings[i].year,
          date: this.shippings[i].date,
          competitionId: this.shippings[i].competitionId,
          shippingId: this.shippings[i].shippingId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-shippings',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForShippingsChanged', this.searchQuery);
		//this.renderShippingsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalShippings = false;

      const currentDate = new Date().getTime();
      this.shippingToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.shippingToAdd);
      console.log(jsonData);
      
      const res = await ShippingService.addShipping(jsonData);
