					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateReturnRequest()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2ReturnRequests = true">Add</base-button>
								              <modal :show.sync="modal2ReturnRequests">
								  ...
								  <base-input label="Status" type="text" placeholder="Enter Status" v-model="returnRequestToAdd.status"></base-input>
												                  </form>
								                </div>
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Return Request Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="ReturnRequestId" type="text" placeholder="Enter ReturnRequestId" v-model="returnRequestDetails.returnRequestId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Request Date</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<datepicker v-model="returnRequestDetails.requestDate"  placeholder="Select Date" ></datepicker>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Status</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="Status" type="text" placeholder="Enter Status" v-model="returnRequestDetails.status"></input>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  methods: {
    ...
    async handleAddSubmitted() {
      this.modal2ReturnRequests = false;
    ...
    async updateReturnRequest() {
      ...
      const jsonData = JSON.stringify(this.returnRequestDetails);
      const res = await ReturnRequestService.update(jsonData);
...
//        this.getAllReturnRequests();
    ...
    async getReturnRequestDetails() {
      try {
			let response = await ReturnRequestService.get(this.returnRequestId);
			this.returnRequestDetails = response.data;
