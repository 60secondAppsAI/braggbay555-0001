					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateOrderItem()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2OrderItems = true">Add</base-button>
								              <modal :show.sync="modal2OrderItems">
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Order Item Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="OrderItemId" type="text" placeholder="Enter OrderItemId" v-model="orderItemDetails.orderItemId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Quantity</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="Quantity" type="text" placeholder="Enter Quantity" v-model="orderItemDetails.quantity"></input>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  methods: {
    ...
    async updateOrderItem() {
      ...
      const jsonData = JSON.stringify(this.orderItemDetails);
      const res = await OrderItemService.update(jsonData);
...
//        this.getAllOrderItems();
    ...
    async getOrderItemDetails() {
      try {
			let response = await OrderItemService.get(this.orderItemId);
			this.orderItemDetails = response.data;
